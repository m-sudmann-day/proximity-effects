            ...
            if ($isPost && $selected_area_id != -1 && $selected_category_id != -1)
            {
                ...
                foreach(glob($files) as $file)
                {
                    if (!$atLeastOneFile)
                    {
                        ...
                        echo("<tr><td colspan='2'>2) <b>Standardized Median Absolute Deviation of Rating vs. Distance</b>: The ratings that we assign to each business after standardizing individual reviewers and individual review.  The Y-axis represents the businesses' number of median absolute deviations (MAD).</td></tr>");
                        echo("<tr><td colspan='2'>3) <b>Difference Between Standardized MAD Rating of Closest Competitors vs. Distance</b>: Again each point is a business, but the Y-axis is the difference between the number of deviations from the median that the two business have, or the difference in consumer sentiment.  The red fit line depicts the correlation of distance between competitors to difference in consumer sentiment.</td></tr>");
                        echo("<tr><td colspan='2'>4) <b>Difference Between Standardized MAD Rating Against 3 Closest Competitors vs. Distance</b>: This is similar to the previous chart, but typically a diluted result.  The distances and ratings of the three closest competitors are merged into a single value using inverse distance weighting then compared against the plotted businesses to generated the Y-axis value.</td></tr>");
                        echo("<tr><td colspan='2'>5) <b>Standard Deviation of Standardized MAD Ratings vs. Distance</b>:  This chart digs one level deeper and looks at the standard deviation of individual standardized reviews for each business plotted against the distance to the nearest competitor.  The fit line shows the correlation between this level of variance and the distance to the closest competitor.</td></tr>");
                        echo("<tr><td colspan='2'>Interpretation of the results is left to the reader.  The slopes and significances vary greatly between business categories and somewhat between geographies.  The title of each chart shows how many businesses are included in the analysis.</td></tr>");
                        $atLeastOneFile = TRUE;
                    }
                    echo("<tr><td>&nbsp;</td></tr>");
